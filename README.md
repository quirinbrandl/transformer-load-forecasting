...
This project uses [uv](https://docs.astral.sh/uv/) for dependency management. Make sure it is installed on your system. Then run
...
This will create a new virtual environment and install all needed packages into that.
Now you just need to activate the virtual envrionment via
...
By default, the data from [The Building Data Genome 2 Data-Set](https://github.com/buds-lab/building-data-genome-project-2/tree/master/data) should be placed under `data/building-data-genome-project-2-master`.
The path to the dataset files can be changed in `src/config.yml`. 3. The parameters of the experiments can be set up in the config file `src/config.yml`.
